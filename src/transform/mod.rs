...
impl<'a> CodeTransform<'a> {
    ...
    fn transform_block(
        ...
        loop {
            let sub_blocks = {
                let modules = &self.ctx.lock_modules();
                if !cursor.has_next(&modules[mod_idx]) {
                    break;
                }
                cursor.sub_blocks(&modules[mod_idx])
            };

            if let Some((first, second)) = sub_blocks {
                tracing::trace!("transforming nested block {first}");
                self.transform_block(step, mod_idx, first);
                if let Some(second) = second {
                    tracing::trace!("transforming nested block {second}");
