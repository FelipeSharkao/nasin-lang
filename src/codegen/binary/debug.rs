...
impl<'a> DebugData<'a> {
    ...
    pub fn write_debug_sections(&mut self, object: &mut Object) {
        ...
        dwarf.unit.get_mut(root).set(
            ...
            AttributeValue::String((*b"nasin").into()),
        );
        dwarf.unit.get_mut(root).set(
            gimli::DW_AT_name,
            AttributeValue::String(self.cfg.name.as_bytes().to_vec()),
        );
        dwarf.unit.get_mut(root).set(
            gimli::DW_AT_comp_dir,
            AttributeValue::String(base_dir.to_string_lossy().as_bytes().to_vec()),
        );
        ...
        for i in 0..self.funcs.len() {
            ...
            self.add_subprogram(&mut dwarf, &mut line_program, root, i, symbol_info);
        }
    ...
    fn add_subprogram(
        ...
        let die = dwarf.unit.add(parent, gimli::DW_TAG_subprogram);

        // DW_AT_linkage_name is not set because GDB will prefer it, and read
        // DW_AT_language to get the de-mangling scheme, so for us it shows the mangled
        // name, which is not what we want.
        dwarf.unit.get_mut(die).set(
            gimli::DW_AT_name,
            AttributeValue::String(func.name.to_string().into_bytes()),
        );
