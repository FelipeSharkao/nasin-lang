...
impl<'a> FuncCodegen<'a, '_> {
    ...
    pub fn add_block(
        ...
        let body = self.ctx.modules[mod_idx].block(block_idx).to_vec();
        for instr in &body {
    ...
    pub fn add_instr(
        ...
        match &instr.body {
            ...
            b::InstrBody::If(cond, then_block_idx, else_block_idx) => {
                let then_body = self.ctx.modules[mod_idx].block(*then_block_idx);
                let else_body = self.ctx.modules[mod_idx].block(*else_block_idx);
